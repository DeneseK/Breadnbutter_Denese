VERSION 5.00
Begin {82282820-C017-11D0-A87C-00A0C90F29FC} RWeeklySales 
   ...
   _ExtentX        =   22251
   _ExtentY        =   13811
   SectionData     =   "RWeeklySales.dsx":0000
End
Attribute VB_Name = "RWeeklySales"
...
Public Sub Setup(pCN As ADODB.Connection, pdStartDate As Date, pdEndDate As Date)
  ...
  If ConnType = Access Then
    adc.Source = "SELECT * FROM QWeeklySales WHERE Date Between #" & pdStartDate & "# AND #" & pdEndDate & "# ORDER BY Date"
  Else
    adc.Source = "SELECT TCompany.Name AS Company, " & _
      "[FirstName] + ' ' + [LastName] AS Name, " & _
      "TContact.Copies, TSupportActs.Type, TSupportActs.Date, TSupportActs.[User] " & _
      ...
      "WHERE (((TSupportActs.Type) = 'Sale')) " & _
